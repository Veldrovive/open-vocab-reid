...
class Wildlife10kDataset(AbstractVideoReIDDataset):
    def __init__(
        ...
        if self.load_segmentations:
            assert self.sidecar_root is not None, "Wildlife10kDataset sidecar root must be specified for loading segmentations."

        if self.download_if_missing:
            self._download_dataset()
        else:
            assert self.ds_root.exists(), f"Wildlife10k dataset not found at {self.ds_root}"
    ...
    def _prepare_dataset(self):
        ...
        if self.verbose:
            iterator = tqdm(iterator, total=len(df), desc="Processing Wildlife10kDataset")
